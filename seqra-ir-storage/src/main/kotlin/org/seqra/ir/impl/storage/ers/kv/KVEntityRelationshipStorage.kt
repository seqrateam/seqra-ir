...
import org.seqra.ir.impl.storage.ers.EntityRelationshipStorageBase
...
class KVEntityRelationshipStorage(private val kvStorage: PluggableKeyValueStorage) : EntityRelationshipStorageBase() {
    ...
    internal fun getEntityTypeId(
        type: String,
        kvTxn: org.seqra.ir.api.storage.kv.Transaction
    ): Int? {
    ...
    internal fun getEntityTypes(kvTxn: org.seqra.ir.api.storage.kv.Transaction): Map<String, Int> {
    ...
    private fun entityTypesMap(txn: org.seqra.ir.api.storage.kv.Transaction, create: Boolean): NamedMap? {
        return entityTypesMap ?: txn.getNamedMap(entityTypesMapName, create)?.also {
            entityTypesMap = it
        }
    }

    internal fun entityCountersMap(txn: org.seqra.ir.api.storage.kv.Transaction, create: Boolean): NamedMap? {
    ...
    internal fun deletedEntitiesMap(
        typeId: Int,
        txn: org.seqra.ir.api.storage.kv.Transaction,
        create: Boolean
    ): NamedMap? {
...
private const val packageNamePrefix = "org.seqra.ir.impl.storage.ers.kv#"
